...
void startup()
{
    ...
    SetSoundVolume(complete_line_sfx, 0.15f);
...
void render()
{
    BeginDrawing();
        ...
        if (is_pause)
        {
            DrawRectangle(SCREEN_WIDTH / 2 - 50 + 20, SCREEN_HEIGHT / 2 - 20, 10, 35, RAYWHITE);
            DrawRectangle(SCREEN_WIDTH / 2 - 50 + 40, SCREEN_HEIGHT / 2 - 20, 10, 35, RAYWHITE);
        }
        BeginMode3D(camera);
...
void event_handler()
{
    ...
    {
        if (!is_pause)  PauseMusicStream(music);
